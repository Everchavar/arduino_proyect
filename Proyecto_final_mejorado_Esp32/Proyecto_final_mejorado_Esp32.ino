...
// Pines de relés y NOMBRES ASOCIADOS
int relays[] = { 23, 22, 21, 19 };
// AÑADIDO: Lista de nombres que deseas mostrar en el HTML
const char* relayNames[] = { "Sala", "Comedor", "Cochera", "Cuarto" };
...
// =======================================================
// 3. PÁGINA DE CONTROL (Minimizada con Nuevos Estilos)
// =======================================================
const char CONTROL_HTML_HEADER[] PROGMEM = R"rawliteral(
<!DOCTYPE html><html><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Control de Luces</title><style>
/* FONDOS NEGROS */
body{font-family:'Arial',sans-serif;background-color:#000000;color:#fff;display:flex;flex-direction:column;align-items:center;padding:20px;min-height:100vh}
h2{color:#f5f5f5;margin-bottom:30px;font-weight:500}
.control-card{background-color:#1c1c1c;padding:20px;border-radius:8px;box-shadow:0 4px 8px rgba(0,0,0,.4);width:100%;max-width:300px;text-align:center}
.relay-group{background-color:#1c1c1c;padding:15px 0;margin-bottom:15px;width:100%;display:flex;justify-content:space-between;align-items:center;border-bottom:1px solid #555}
/* CONTENEDOR DE TEXTO E ÍCONO */
.relay-text-container{display:flex;align-items:center;flex-grow:1;text-align:left;font-size:1.1em;font-weight:400;color:#f5f5f5}
/* ÍCONO DE BOMBILLA (ANIMACIÓN) */
.light-icon{margin-right:10px;font-size:1.4em;filter:grayscale(100%) brightness(30%);transition:filter 0.4s ease}
.is-on .light-icon{filter:grayscale(0%) brightness(150%) drop-shadow(0 0 8px #FFD700)}
/* SWITCH TOGGLE */
...
input:checked+.slider{background-color:#00A3FF} /* CAMBIO A CELESTE */
input:checked+.slider:before{transform:translateX(26px)}
/* BOTÓN DE LOGOUT (CELESTE Y CURVADO) */
.logout-btn{margin-top:25px;background:#007bff;color:white;padding:10px 20px;border-radius:10px;text-decoration:none;display:block;transition:background-color .3s}
.logout-btn:hover{background:#0056b3}
</style><script>
/* FUNCIÓN CORREGIDA: Ahora actualiza la UI localmente (foco) y envía el comando al ESP32 */
function toggleRelay(id,chk){fetch('/relay?ch='+id+'&state='+(chk.checked?1:0));const i=chk;const r=i.closest('.relay-group');const c=r.querySelector('.relay-text-container');if(chk.checked){c.classList.add('is-on');c.classList.remove('is-off')}else{c.classList.add('is-off');c.classList.remove('is-on')}}
</script></head><body><div class="control-card"><h2>Control de Luces</h2>
)rawliteral";
...
// =======================================================
// 4. DEFINICIÓN DE FUNCIONES (Actualizada)
// =======================================================
...
void sendControlPage() {
  ...
  for (int i = 0; i < NUM_RELAYS; i++) {
    ...
    // OBTENEMOS EL NOMBRE DESCRIPTIVO
    String name = relayNames[i];
    
    // ESTADO PARA LA ANIMACIÓN Y CHECKBOX
    String stateClass = (digitalRead(pin) == LOW) ? "is-on" : "is-off";
    String checkedAttr = (digitalRead(pin) == LOW) ? "checked" : "";

    page += "<div class=\"relay-group\">";
    
    // CONTENEDOR CON EL ÍCONO DE LA BOMBILLA Y EL NOMBRE
    page += "<div class=\"relay-text-container " + stateClass + "\">";
    page += "<span class=\"light-icon\">💡</span>"; // Ícono de bombilla Unicode
    page += "<span>" + name + "</span>";          // Nombre del relé
    page += "</div>";

    page += "<label class=\"switch\">";
    // Inyecta 'checked' si el relé está ON
    page += "<input type=\"checkbox\" onchange=\"toggleRelay(" + String(channel) + ", this)\" " + checkedAttr + ">";
